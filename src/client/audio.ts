...
import { BROWN_NOISE } from "./worklet.ts";

const WORKLET_URL = "/js/client/worklet.js";

/** Safari는 접두사가 붙은 생성자만 노출하던 시절이 있다. */
function createAudioContext(): AudioContext {
  const Ctor =
    window.AudioContext ??
    (window as unknown as { webkitAudioContext?: typeof AudioContext }).webkitAudioContext;
  if (Ctor === undefined) throw new Error("이 브라우저는 Web Audio API를 지원하지 않습니다");
  return new Ctor();
}
 ...
 *
 */
export function gainForTemperature(
  temp: number,
  min: number,
  max: number,
  deviceKind: DeviceKind = "aircon",
): number {
...
export class BrownNoise extends EventTarget {
  ...
  #source: AudioWorkletNode | null = null;
   ...
   * @returns 실제로 시작됐는지
   */
  async start(gainValue: number): Promise<boolean> {
    ...
    try {
      ...
      this.#source = new AudioWorkletNode(context, PROCESSOR_NAME, {
      ...
      this.#source.connect(this.#filter).connect(this.#gain).connect(context.destination);
      return true;
    } catch (error) {
  ...
  #disconnectAll(nodes: readonly (AudioNode | null)[]): void {
