...
/** socket.io 핸드셰이크에서 우리가 실제로 읽는 부분만 추린 타입. */
export type HandshakeLike = {
...
export type ClientIpOptions = {
  /**
   * 신뢰하는 리버스 프록시 홉 수. 0이면 X-Forwarded-For를 아예 무시한다.
   */
...
export function normalizeIp(value: unknown): string | null {
  ...
  if (bracketed?.[1] !== undefined) ip = bracketed[1];
 ...
 *
 * @returns 항상 비어 있지 않은 문자열
 */
export function resolveClientIp(
  handshake: HandshakeLike | undefined,
  { trustProxyHops }: ClientIpOptions,
): string {
  ...
  if (trustProxyHops > 0) {
    ...
    if (chain.length > 0) {
      ...
      const index = Math.min(Math.max(0, chain.length - trustProxyHops), chain.length - 1);
      return chain[index] ?? direct ?? UNKNOWN_IP;
    }
  }
