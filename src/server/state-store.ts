...
import { type PersistedState, persistedStateSchema } from "../shared/protocol.ts";

export type StateStoreOptions = {
  file: string;
  debounceMs?: number | undefined;
  logger?: StoreLogger | undefined;
  enabled?: boolean | undefined;
};

/** 실제로 쓰는 메서드만. Fastify의 pino 로거와 console 둘 다 만족한다. */
...
export class StateStore {
  ...
  constructor({ file, debounceMs = 2000, logger = console, enabled = true }: StateStoreOptions) {
    this.#file = file;
   ...
   *
   * 디스크의 JSON은 우리가 쓴 것이라도 신뢰할 수 없다(손상, 손편집, 예전
   * 버전의 형식). 스키마로 검증해서 통과한 것만 쓴다.
   */
  async load(): Promise<PersistedState | null> {
    if (!this.#enabled) return null;
    try {
      const raw = await readFile(this.#file, "utf8");
      const parsed = persistedStateSchema.parse(JSON.parse(raw), "state.json");
      if (!parsed.ok) {
        this.#logger.warn?.(
  ...
  schedule(data: PersistedState): void {
  ...
  async #write(data: PersistedState): Promise<void> {
