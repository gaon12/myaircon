...
import { GAIN, gainForTemperature, PEAK_HEADROOM_SIGMA } from "../public/js/audio.js";
import { BROWN_NOISE, renderBrownNoise } from "../public/js/worklet.js";

const BLOCK = 128;

/** 워크렛이 실제로 하는 것처럼 블록 단위로 이어 붙여 렌더링한다. */
function renderChannel(blocks, random = Math.random) {
...
describe("브라운 노이즈 DSP", () => {
  ...
  it("블록 경계에서 상태가 이어진다 (뚝뚝 끊기지 않는다)", () => {
    ...
    assert.ok(Math.abs(next[0] - last) < 0.01);
  });
...
describe("게인 스테이징", () => {
  it("온도가 높을수록 조용해진다", () => {
    ...
    for (let i = 1; i < gains.length; i++) {
      assert.ok(gains[i] < gains[i - 1], `${gains[i]} < ${gains[i - 1]}`);
    }
  ...
  it("범위를 벗어난 값과 이상한 입력을 클램프한다", () => {
    ...
    for (const bad of [Number.NaN, Number.POSITIVE_INFINITY, undefined, null, "x"]) {
      const g = gainForTemperature(bad, 18, 30);
