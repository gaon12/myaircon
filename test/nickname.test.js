...
const n = (raw, opts = OPTS) => normalizeNickname(raw, opts);
...
describe("normalizeNickname", () => {
  ...
  it("이모지를 grapheme 경계에서 자른다 (기존 substring은 surrogate pair를 반토막 냈다)", () => {
    const out = n("🎉".repeat(20));
    assert.equal(graphemes(out), 9);
    assert.ok(!out.includes("\uFFFD"), "치환 문자가 섞이면 안 된다");
    assert.equal(out, "🎉".repeat(9));
  });

  it("ZWJ로 이어진 가족 이모지를 한 글자로 세고 쪼개지 않는다", () => {
    const family = "👨‍👩‍👧‍👦";
    const out = n(family + "🎉".repeat(20));
    assert.equal(graphemes(out), 9);
    assert.ok(out.startsWith(family), "가족 이모지가 온전히 남아야 한다");
  });
